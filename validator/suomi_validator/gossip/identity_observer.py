...
from suomi_validator.journal.event_extractors import \
    BlockEventExtractor
from suomi_validator.journal.event_extractors import \
...
class IdentityObserver(ChainObserver):
    ...
    def chain_update(self, block, receipts):
        """
        Handles both "suomi/block-commit" Events and "identity/update"
        Events. For "suomi/block-commit", the last_block_num is updated or a
        fork is detected. For "identity/update", the corresponding cache entry
        will be updated.
        """

        block_events = BlockEventExtractor(block).extract([
            EventSubscription(event_type="suomi/block-commit")])

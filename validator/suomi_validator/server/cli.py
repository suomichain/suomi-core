...
from suomi_validator.config.path import load_path_config
from suomi_validator.config.validator import load_default_validator_config
from suomi_validator.config.validator import load_toml_validator_config
from suomi_validator.config.validator import merge_validator_config
from suomi_validator.config.validator import ValidatorConfig
from suomi_validator.config.logs import get_log_config
from suomi_validator.server.core import Validator
from suomi_validator.server.keys import load_identity_signer
from suomi_validator.server.log import init_console_logging
from suomi_validator.server.log import log_configuration
from suomi_validator.server.state_verifier import verify_state
from suomi_validator.exceptions import GenesisError
from suomi_validator.exceptions import LocalConfigurationError
from suomi_validator import metrics


LOGGER = logging.getLogger(__name__)
DISTRIBUTION_NAME = 'suomi-validator'


def parse_args(args):
    parser = argparse.ArgumentParser(
        description='Configures and starts a Suomi validator.',
    ...
    parser.add_argument(
        '-V', '--version',
        action='version',
        version=(DISTRIBUTION_NAME + ' (Suomi) version {}')
        .format(version),
...
def main(args=None):
    ...
    try:
        ...
        version = 'UNKNOWN'
    LOGGER.info(
        '%s (Suomi) version %s', DISTRIBUTION_NAME, version)

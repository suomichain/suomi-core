...
class ConsensusFactory(object):
    ...
    def get_consensus_module(module_name):
        ...
        if module_name == 'genesis':
            module_package = (
                'suomi_validator.journal.consensus.genesis.'
                'genesis_consensus'
            )
        elif module_name == 'devmode':
            module_package = (
                'suomi_validator.journal.consensus.dev_mode.'
                'dev_mode_consensus'
            )
        elif module_name == 'poet':
            module_package = 'suomi_poet.poet_consensus'
    ...
    def get_configured_consensus_module(block_id, state_view):
        ...
        consensus_module_name = settings_view.get_setting(
            'suomi.consensus.algorithm', default_value=default_consensus)

...
from suomi_validator.server.events.extractor import EventExtractor
from suomi_validator.protobuf.events_pb2 import Event
from suomi_validator.protobuf.transaction_receipt_pb2 import StateChangeList
...
class BlockEventExtractor(EventExtractor):
    ...
    def _make_event(self):
        ...
        return Event(event_type="suomi/block-commit", attributes=attributes)

    def extract(self, subscriptions):
        if subscriptions:
            for sub in subscriptions:
                if sub.event_type == "suomi/block-commit":
...
class ReceiptEventExtractor(EventExtractor):
    ...
    def _make_state_delta_events(self, subscriptions):
        gen = False
        for subscription in subscriptions:
            if subscription.event_type == "suomi/state-delta":
                gen = True

...
from suomi_sdk.processor.handler import TransactionHandler
from suomi_sdk.messaging.future import FutureTimeoutError
from suomi_sdk.processor.exceptions import InvalidTransaction
from suomi_sdk.processor.exceptions import InternalError

from suomi_settings.protobuf.settings_pb2 import SettingsPayload
...
from suomi_settings.protobuf.setting_pb2 import Setting
...
def _get_setting_candidates(context):
    value = _get_setting_value(context, 'suomi.settings.vote.proposals')
    if not value:
...
def _get_approval_threshold(context):
    return int(_get_setting_value(
        context, 'suomi.settings.vote.approval_threshold', 1))


def _get_auth_keys(context):
    value = _get_setting_value(
        context, 'suomi.settings.vote.authorized_keys', '')
...
def _validate_setting(auth_keys, setting, value):
    ...
    if setting == 'suomi.settings.vote.proposals':
        raise InvalidTransaction(
            'Setting suomi.settings.vote.proposals is read-only')
